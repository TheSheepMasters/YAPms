...
function updateArticles() {
	fetch("./api/news/get_news.php")
	.then(response => response.json())
	.then(data => {
		const articles = document.getElementById("yapnews-articles");
		for(const articleData of data) {
			...
			articleTitle.setAttribute('href', articleData['source']);
			...
			articleAuthor.innerHTML = articleData['author'];
			articleSnippet.innerHTML = articleData['snippet'];
...
function updateMobile() {
	...
	for(let index = 0; index < clickButtons.length; ++index) {
...
function start() {
	...
	if(php_load_map === true) {
		...
		if(php_load_user === true) {
			url = "./www-data/users/" + php_load_user_id + "/" + php_load_map_id + ".txt.gz"; 	
		} else {
			url = "./www-data/maps/" + php_load_map_id + ".txt.gz";
		}
